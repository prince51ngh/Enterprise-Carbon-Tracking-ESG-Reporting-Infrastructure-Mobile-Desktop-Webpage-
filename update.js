...
const dir = __dirname;
const files = ['about.html', 'contact.html', 'pricing.html', 'impact.html', 'resources.html', 'feature-carbon.html', 'feature-finance.html', 'estimator.html', 'compliance.html'];

const metaTags = `    <meta property="og:title" content="VerdeLedger // Integrated Sustainability Accounting">
...
for (let file of files) {
  ...
  if (!html.includes('og:title')) {
    html = html.replace(/<title>VerdeLedger \/\/ /g, metaTags);
  }
  
  // Insert Favicon
  if (!html.includes('favicon.svg')) {
    html = html.replace(/<link rel="preconnect"/g, faviconLink);
  }

  // Update Header Logo
  html = html.replace(/<a href="index\.html" class="site-brand">Verde<span>Ledger<\/span><\/a>/g, headerLogo);

  // Add Theme Toggle
  if (!html.includes('id="theme-toggle"')) {
    html = html.replace(/<\/ul>\s*<\/nav>/, themeToggle);
  }

  // Add Scroll Progress
  if (!html.includes('scroll-progress-container')) {
    html = html.replace(/<\/header>\s*<main class="page-body">/, scrollBar);
  }

  // Replace Footer
  if (!html.includes('footer-rich')) {
